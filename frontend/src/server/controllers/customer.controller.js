...
import { mapDoc, mapDocs, mapDepartments } from '../utils/transform.js';
...
export const getDepartments = async (req, res) => {
  ...
  res.json({ success: true, data: mapDepartments(departments) });
};
...
export const getDocuments = async (req, res) => {
  const customerId = req.user._id;
  const filters = { customer_id: customerId, is_deleted: false };
...
export const downloadDocument = async (req, res) => {
  ...
  if (type === 'result') {
    ...
    if (doc.direction === 'response' && doc.payment_blocked) {
      throw new AppError('This response is blocked. Please contact the firm.', 403);
    }
  }
