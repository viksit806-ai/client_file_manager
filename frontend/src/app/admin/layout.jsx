...
export default function AdminLayout({ children }) {
  ...
  useEffect(() => {
    const loadRecent = () => {
      const items = JSON.parse(localStorage.getItem('recent_admin') || '[]');
          ...
          <div className="flex items-center mb-3">
            ...
            <div className={`transition-all duration-300 whitespace-nowrap overflow-hidden ${sidebarOpen ? 'opacity-100 max-w-64 ml-3' : 'opacity-0 max-w-0 ml-0 pointer-events-none'}`}>
              <div className="text-sm truncate">
                ...
                <p className="text-xs text-navy-300/80">Admin</p>
              </div>
            </div>
          </div>
          <Link
            ...
            onClick={closeSidebar}
            className="flex items-center text-sm text-navy-300 hover:text-white w-full px-2 py-1.5 rounded hover:bg-navy-800/40 overflow-hidden mb-1"
          >
