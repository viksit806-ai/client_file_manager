    ...
    <div className="flex flex-col -m-6 h-screen bg-white select-none overflow-hidden">
          ...
          {currentPath.map((item, idx) => (
            <div key={idx} className="flex items-center gap-1">
              ...
              <button
                ...
                className={`hover:text-blue-600 truncate max-w-[120px] ${idx === currentPath.length - 1 ? 'text-slate-800 font-semibold' : ''}`}
              >
                {item.name}
              </button>
            </div>
      ...
      <div className="flex flex-row flex-1 overflow-hidden">
        ...
        <div className="w-52 bg-slate-50/50 border-r border-slate-200/60 h-full overflow-y-auto shrink-0 flex flex-col p-3 text-xs text-slate-700">
          ...
          <div>
            <span className="font-semibold text-slate-400 block px-2 mb-1.5 uppercase text-[9px] tracking-wider">Departments</span>
